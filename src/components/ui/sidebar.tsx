...
function SidebarProvider({
  ...
  const setOpen = useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // biome-ignore lint/suspicious/noDocumentCookie: explicitly accept cookie assignment here
  ...
  useEffect(() => {
    if (!SIDEBAR_KEYBOARD_SHORTCUT_ENABLED) {
      return;
    }
...
function Sidebar({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-150 ease-snappy data-[side=right]:right-0 data-[side=left]:left-0 data-[side=right]:group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)] data-[side=left]:group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)] md:flex",
